...
class ShaderProgram(vertexShaderPath: String, fragmentShaderPath: String) {
    ...
    fun setUniformf(name: String, value: Float): Boolean {
        use()
        val loc = GL20.glGetUniformLocation(programID, name)
        return if (loc != -1) {
    ...
    fun setUniform4f(name: String, matrix: Matrix4f, transpose: Boolean): Boolean {
        use()
        val loc = GL20.glGetUniformLocation(programID, name)
        return if (loc != -1) {
            //matrix[m4x4buf]
    ...
    fun setUniform2f(name: String, vector2f: Vector2f): Boolean {
        use()
        val loc = GL20.glGetUniformLocation(programID, name)
        return if (loc != -1) {
            GL20.glUniform2f(loc, vector2f.x, vector2f.y)
             true
        }
        else false
    }

    fun setUniform3f(name: String, vector3f: Vector3f): Boolean {
        use()
        val loc = GL20.glGetUniformLocation(programID, name)
        return if (loc != -1) {
            GL20.glUniform3f(loc, vector3f.x, vector3f.y, vector3f.z)
            true
        }
        else false
    }
